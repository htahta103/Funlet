...
async function sendSMS(phoneNumber, message, shouldSend = true, owner_phone_number = null) {
  try {
...
function checkCreateCrewPattern(message) {
...
function checkCheckRsvpsPattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
...
function checkManageEventPattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
  const originalMessage = message.trim();
...
function checkSendMessagePattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
  const patterns = [
    /^send\s+message$/,
    /^message$/,
  ...
  for (const pattern of patterns){
    if (normalizedMessage.match(pattern)) return {
      isMatch: true
    };
  }
  return {
...
async function checkCheckCrewMembersPattern(message, supabase, userId) {
  const normalizedMessage = message.toLowerCase().trim();
  ...
  if (standaloneNamePattern.test(message.trim()) && message.trim().length >= 2) {
    ...
    if (supabase && userId) {
      ...
      const { data: crewData, error: crewError } = await supabase.from('crews').select('id, name').eq('creator_id', userId).ilike('name', crewName).maybeSingle();
...
function checkSendInvitationsPattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
...
function checkInviteMorePeoplePattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
...
function checkSyncUpPattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
...
function checkReSyncPattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
...
function checkHelpPattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
  const helpPatterns = [
    /^help$/,
    /^menu$/,
...
function isSkippableMessage(message) {
...
function checkSyncUpDetailsInputPattern_DEPRECATED(message) {
...
function checkSyncStatusPattern(message) {
  const patterns = [
    /^sync\s+check(?:\s+(.+))?$/i,
    /^sync\s+status(?:\s+(.+))?$/i,
...
function parseReSyncTimeOptions(message) {
...
function parseSyncUpDetails_DEPRECATED(message) {
  // Split by comma
...
function parseTimeOption(text, idx) {
...
function checkAddCrewMembersPattern(message) {
  const normalizedMessage = message.toLowerCase().trim();
...
function hasMixedInput(message) {
...
function parseMemberInfo(text) {
  ...
  const normalizedText = text.replace(/[\u200E\u200F\u202A-\u202E\u2066-\u2069]/g, '') // Remove directional marks
  .trim();
...
// Helper function to convert day names and month dates to YYYY-MM-DD format
function convertDayNameToDate(dayName) {
  const dayMap = {
    'sunday': 0,
    'monday': 1,
    'tuesday': 2,
...
function formatTimeRangeForOptionGlobal(startIso, endIso) {
  ...
  if (!end) {
    return {
      dayMonth: `${weekday} ${monthDay}`,
      timeText: `${formatLower(start)}`
    };
  }
...
async function formatSyncUpConfirmation(syncUpData, supabaseClient) {
  // Count crew members (robust: use exact count, fallback to data length)
  let memberCount = 0;
  try {
    const { data, count } = await supabaseClient.from('crew_members').select('id', {
      count: 'exact'
  ...
  parts.push('\nTime options:');
  (syncUpData.time_options_parsed || []).forEach((opt)=>{
...
async function buildSyncUpRecipientList(supabase, syncUpId, targetingGroup) {
  try {
    if (targetingGroup === 'everyone') {
      // Everyone: all contacts who have a sync_up_response row for this sync up
      const { data: responses, error: responsesError } = await supabase.from('sync_up_responses').select(`
          contact_id,
          option_ids,
          contacts!inner(
            id,
      ...
      const nonResponders = (responses || []).filter((r)=>!r.option_ids || r.option_ids.length === 0);
      return nonResponders.map((r)=>({
          id: r.contacts.id,
          name: [
            r.contacts.first_name,
            r.contacts.last_name
...
function formatTimeOptions(syncUpOptions) {
...
async function fetchEventWithShortUrl(supabase, eventId, maxRetries = 5, retryDelay = 1000) {
  let retryCount = 0;
  while(retryCount < maxRetries){
    const { data: eventData, error: eventError } = await supabase.from('events').select('id, title, event_date, start_time, shorten_event_url').eq('id', eventId).single();
    if (eventError) {
      console.error(`Error fetching event (attempt ${retryCount + 1}/${maxRetries}):`, eventError);
      return {
        shorten_event_url: null,
        event: null
      };
    }
    if (eventData && eventData.shorten_event_url) {
      ...
      return {
        shorten_event_url: eventData.shorten_event_url,
        event: eventData
      };
    }
  ...
  const { data: eventData } = await supabase.from('events').select('id, title, event_date, start_time, shorten_event_url').eq('id', eventId).single();
  return {
    shorten_event_url: null,
    event: eventData
  };
...
function formatEventLink(eventId, shortenEventUrl) {
  if (shortenEventUrl) {
...
function formatPhoneNumberForCrewMessage(phone) {
  // Remove all non-digits
  const digits = phone.replace(/\D/g, '');
  // If it starts with 1, remove it
  const cleanDigits = digits.startsWith('1') && digits.length === 11 ? digits.substring(1) : digits;
...
function formatEventTime(timeString, endTimeString) {
...
async function getCrewJoinLink(supabase, crewId) {
  const { data: crewData } = await supabase.from('crews').select('invite_url, invite_code').eq('id', crewId).single();
...
async function validateCrewOwnership(supabase, crewId, userId) {
  const { data: crew, error } = await supabase.from('crews').select('id, name, creator_id').eq('id', crewId).single();
  if (error || !crew) {
    return {
      isValid: false,
      crew: null
...
function formatCrewMembersDisplay(crewName, members, totalCount) {
  if (totalCount === 0) {
...
function formatManagementMenu(crewName) {
...
async function getCrewMembersWithPagination(supabase, crewId, page = 0, pageSize = 5) {
  // Get total count
  const { count: totalCount } = await supabase.from('crew_members').select('*', {
    count: 'exact',
    head: true
  }).eq('crew_id', crewId);
  // Get paginated members
  const { data: members, error } = await supabase.from('crew_members').select(`
      id,
      contact_id,
      contacts (
        id,
    ...
    `).eq('crew_id', crewId).order('added_at', {
    ascending: false
...
async function searchUserContacts(supabase, userId, searchQuery) {
  const { data: contacts, error } = await supabase.from('contacts').select('id, first_name, last_name, phone_number').eq('user_id', userId).ilike('first_name', `%${searchQuery}%`).order('first_name', {
    ascending: true
  });
  if (error) {
...
async function formatContactDisplay(contact) {
...
async function showContactActionsMenu(supabase, userId, phone_number, send_sms1, contactId, phoneNumberForState) {
  // Get contact details
  const { data: contact } = await supabase.from('contacts').select('first_name, last_name, phone_number').eq('id', contactId).eq('user_id', userId).single();
  if (!contact) {
  ...
  await sendSMS(phone_number, response, send_sms1, phone_number);
  return response;
}
// Helper function to show crew members and management menu
async function showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, crewId, crewName, phoneNumberForState) {
  ...
  if (!ownership.isValid) {
    ...
    await sendSMS(phone_number, errorMsg, send_sms1, phone_number);
    return errorMsg;
  }
  // Fetch latest crew name from database to ensure we have the current name
  const { data: crewData, error: crewError } = await supabase.from('crews').select('name').eq('id', crewId).single();
  if (crewError) {
  ...
  const { data: allMembers, error: membersError } = await supabase.from('crew_members').select(`
      id,
      contact_id,
      contacts (
        id,
  ...
  await sendSMS(phone_number, responseContent, send_sms1, phone_number);
  return responseContent;
}
// Helper function to show event details and management menu
async function showEventDetailsAndMenu(supabase, userId, phone_number, send_sms1, eventId, phoneNumberForState) {
  // Get event details
  const { data: eventData, error: eventError } = await supabase.from('events').select('id, title, event_date, start_time, end_time, location, notes, status, creator_id').eq('id', eventId).single();
  if (eventError || !eventData || eventData.creator_id !== userId) {
    return 'Event not found or you don\'t have permission to manage it.';
  }
  // Get RSVP summary
  const { data: invitations } = await supabase.from('invitations').select('status').eq('event_id', eventId);
  const rsvpCounts = {
    in: 0,
    out: 0,
    maybe: 0,
...
async function checkPatternMatches(message, currentState = null, isOnboarded = true, userCrewCount = 0, supabase, userId) {
  ...
  if (currentState?.waiting_for === 'duplicate_event_name_input') {
    ...
    if (normalized === 'same') {
      return {
        action: 'DUPLICATE_EVENT_SAME_NAME',
        extractedData: {}
      };
    }
  ...
  if (currentState?.waiting_for === 'edit_contact_phone_input') {
    ...
    if (normalized === 'back') {
      return {
        action: 'EDIT_CONTACT_BACK_TO_MENU',
        extractedData: {}
      };
    }
  ...
  const editContactPatterns = [
    /^fix\s+(.+?)(?:'s)?\s+(?:number|phone|name)/i,
    /^change\s+(.+?)(?:'s)?\s+(?:number|phone|name)/i,
    /^update\s+(.+?)(?:'s)?\s+contact/i,
  ...
  if (currentState?.waiting_for === 'targeting_selection') {
    ...
    if (numericMatch) {
      ...
      return {
        action: 'SEND_MESSAGE',
        extractedData: {
          targeting_index: targetingIndex
        }
      };
    } else {
      return {
  ...
  if (currentState?.waiting_for === 'sync_up_event_selection') {
    ...
    if (numericMatch) {
      ...
      return {
        ...
        extractedData: {
          selection_number: parseInt(numericMatch[1])
        }
      };
    }
  }
  ...
  if (currentState?.waiting_for === 'sync_status_option_selection') {
    ...
    if (numericMatch) {
      return {
        ...
        extractedData: {
          option_number: parseInt(numericMatch[1])
        }
      };
    }
  ...
  if (currentState?.waiting_for === 'sync_status_invite_confirmation') {
    const normalizedMessage = message.toLowerCase().trim();
    if (normalizedMessage === 'yes' || normalizedMessage === 'y' || normalizedMessage === 'confirm' || normalizedMessage === 'yep' || normalizedMessage === 'yeah' || normalizedMessage === 'sure') {
      return {
  ...
  function formatTimeRangeForOption(startIso, endIso) {
    const start = new Date(startIso);
    const end = endIso ? new Date(endIso) : null;
    const weekday = start.toLocaleDateString('en-US', {
      weekday: 'short'
  ...
  if (currentState?.waiting_for === 'sync_up_change_request') {
    ...
    if (eventNameMatch) {
      return {
        ...
        extractedData: {
          eventName: eventNameMatch[4].trim()
        }
      };
    }
  ...
  if (currentState?.waiting_for === 'member_adding_mode') {
    ...
    if (normalizedMessage === 'create crew' || normalizedMessage.startsWith('create crew ')) {
      ...
      if (crewNameMatch) {
        return {
          action: 'CREATE_CREW',
          extractedData: {
            crew_name: crewNameMatch[1].trim()
          }
        };
      } else {
        return {
    ...
    if (hasPhoneNumber) {
      ...
      if (nameOnlyPattern.test(message.trim())) {
        ...
        const invalidMembers = names.map((name)=>({
            name,
            phone: null
          }));
        return {
  ...
  if (createCrewResult.isMatch) {
    return {
      action: 'CREATE_CREW',
      extractedData: {
        crew_name: createCrewResult.crewName
      }
    };
  }
  ...
  if (currentState?.waiting_for === 'crew_selection_manage' || currentState?.waiting_for === 'crew_management_menu' || currentState?.waiting_for === 'remove_members_selection' || currentState?.waiting_for === 'remove_members_pagination' || currentState?.waiting_for === 'rename_crew_input' || currentState?.waiting_for === 'delete_crew_confirmation' || // MANAGE_EVENT states
  currentState?.waiting_for === 'manage_event_selection' || currentState?.waiting_for === 'event_management_menu' || currentState?.waiting_for === 'event_edit_field_selection' || currentState?.waiting_for === 'event_edit_field_input' || currentState?.waiting_for === 'event_edit_continue_or_done' || currentState?.waiting_for === 'event_edit_confirm_changes' || currentState?.waiting_for === 'event_edit_resend_confirmation' || currentState?.waiting_for === 'duplicate_event_name_input' || currentState?.waiting_for === 'duplicate_event_date_input' || currentState?.waiting_for === 'duplicate_event_time_input' || currentState?.waiting_for === 'duplicate_send_invitations' || currentState?.waiting_for === 'delete_event_confirmation' || currentState?.waiting_for === 'delete_event_send_cancellation' || currentState?.waiting_for === 'delete_event_cancellation_message') {
    const normalizedMessage = message.toLowerCase().trim();
    if (normalizedMessage === 'exit' || normalizedMessage === 'quit' || normalizedMessage === 'stop') {
      return {
        action: 'EXIT',
  ...
  if (syncStatusCheck.isMatch) {
    return {
      ...
      extractedData: {
        event_name: syncStatusCheck.eventName
      }
    };
  }
...
async function handleOnboardingContinue(userId, extractedParams, supabase, phoneNumber, substep) {
  try {
    ...
    if (extractedParams.crew_name) {
      ...
      const { data: crewData, error: crewError } = await supabase.from('crews').insert({
        creator_id: userId,
      ...
      if (crewError) {
        ...
        while(retryCount < maxRetries && !inviteUrl){
          ...
          const { data: updatedCrewData, error: fetchError } = await supabase.from('crews').select('id, name, invite_url, invite_code').eq('id', crewData.id).single();
          if (fetchError) {
        ...
        const { data: conversationStateData } = await supabase.from('conversation_state').select('extracted_data').eq('user_id', userId).single();
      ...
      if (crewMembers.length > 0) {
        ...
        for (const member of crewMembers){
          try {
            ...
            const { data: existingContact } = await supabase.from('contacts').select('id, first_name').eq('user_id', userId).eq('phone_number', member.phone).single();
            let contactData;
            if (existingContact) {
              ...
              const { data: updatedContact, error: updateError } = await supabase.from('contacts').update({
              ...
              const { first_name, last_name } = parseNameIntoFirstAndLast(member.name);
              const { data: newContactData, error: contactError } = await supabase.from('contacts').insert({
                user_id: userId,
                first_name: first_name,
                last_name: last_name,
            ...
            const { data: memberData, error: memberError } = await supabase.from('crew_members').insert({
              crew_id: crewId,
...
async function formatSyncUpStatus(supabase, syncUp) {
  // Get all sync up options
  const { data: options } = await supabase.from('sync_up_options').select('*').eq('sync_up_id', syncUp.id).order('idx');
  // Get all responses from sync_up_responses table
  const { data: responses } = await supabase.from('sync_up_responses').select(`
      *,
  ...
  let noneOptionId = null;
  try {
    const { data: noneOpt } = await supabase.from('sync_up_options').select('id').eq('idx', 0).maybeSingle();
  ...
  if (responses && responses.length > 0) {
    responses.forEach((resp)=>{
      const contactName = resp.contacts ? resp.contacts.last_name ? `${resp.contacts.first_name} ${resp.contacts.last_name}` : resp.contacts.first_name : 'Unknown';
...
async function showUninvitedCrewMembers(supabase, event, userId, phoneNumber, send_sms1) {
  try {
    // Get event's crew_id
    const { data: eventData } = await supabase.from('events').select('crew_id, crews(name)').eq('id', event.id).single();
    if (!eventData?.crew_id) {
      return 'Error: No crew found for this event.';
    }
    // Get all crew members
    const { data: crewMembers } = await supabase.from('crew_members').select(`
        contacts (
          id,
          first_name,
          last_name,
    ...
    const { data: invitedMembers } = await supabase.from('invitations').select('contact_id').eq('event_id', event.id);
    const invitedContactIds = new Set(invitedMembers?.map((inv)=>inv.contact_id) || []);
    // Filter out uninvited members
    const uninvitedMembers = crewMembers.filter((member)=>member.contacts && !invitedContactIds.has(member.contacts.id)).map((member)=>{
    ...
    if (uninvitedMembers.length === 0) {
      ...
      const memberNames = uninvitedMembers.length <= 5 ? uninvitedMembers.join(', ') : `${uninvitedMembers.slice(0, 5).join(', ')}...`;
...
const checkRSVPsForEvent = async (supabase, eventId, userId, phoneNumber, send_sms1)=>{
  try {
    // Get event details with shorten_event_url
    const { data: eventData, error: eventError } = await supabase.from('events').select('id, title, event_date, start_time, location, notes, status, shorten_event_url').eq('id', eventId).single();
    if (eventError || !eventData) {
      return 'Sorry, I couldn\'t find that event. Please try again.';
    }
    // Get all invitations for this event with contact details
    const { data: invitations, error: invitationsError } = await supabase.from('invitations').select(`
        id,
        status,
        response_note,
        created_at,
    ...
    console.log('DEBUG: invitations query result:', {
      invitations,
      invitationsError
    });
    ...
    if (phoneNumber) {
      const smsResult = await sendSMS(phoneNumber, rsvpResponse, send_sms1, phoneNumber);
...
const sendMessageForEvent = async (supabase, eventId, userId, phoneNumber, responseContent, shouldSendSMS)=>{
  try {
    ...
    // Get event details
    const { data: eventData, error: eventError } = await supabase.from('events').select('id, title, event_date, start_time, location, notes, status').eq('id', eventId).single();
    console.log('DEBUG: Event query result:', {
      eventData,
      eventError
    });
    if (eventError || !eventData) {
    ...
    console.log('DEBUG: sendMessageForEvent - invitations query result:', {
      invitations,
      invitationsError
    });
    if (invitationsError) {
    ...
    const contactIds = invitations.map((inv)=>inv.contact_id).filter((id)=>id !== null);
    let contactMap = {};
    if (contactIds.length > 0) {
      const { data: contacts, error: contactsError } = await supabase.from('contacts').select('id, first_name, last_name, phone_number').in('id', contactIds);
    ...
    const validInvitations = invitations.filter((invitation)=>invitation.contact_id && contactMap[invitation.contact_id]).map((invitation)=>({
        ...invitation,
    ...
    const { data: updatedState } = await supabase.from('conversation_state').select('current_state, waiting_for, extracted_data').eq('user_id', userId).single();
    return {
      responseContent,
      shouldSendSMS,
      conversationStateData: updatedState
    };
  } catch (error) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { message, phone_number, model = 'gpt-4o-mini', is_host = true, send_sms: send_sms1 = true } = await req.json();
    console.log(`📝 [${Date.now() - startTime}ms] Request parsed`);
    console.log(`🔍 DEBUG: is_host=${is_host}, send_sms=${send_sms1}, message="${message}", phone_number="${phone_number}"`);
    // Initialize response variables early
    let responseContent = '';
    let shouldSendSMS = send_sms1; // Use the send_sms parameter from request
    // Validate input
    if (!message) {
      return new Response(JSON.stringify({
    ...
    if (is_host === true) {
      ...
      if (phone_number) {
        ...
        const plusVariations = phoneVariations.map((phone)=>'+' + phone);
        phoneVariations.push(...plusVariations);
        const { data } = await supabase.from('profiles').select('id, is_onboarded').in('phone_number', phoneVariations).limit(1);
      ...
      if (userId) {
        const { data: stateData, error: stateError } = await supabase.from('conversation_state').select('current_state, onboarding_step, waiting_for, extracted_data').eq('user_id', userId).maybeSingle(); // Use maybeSingle() instead of single() to handle null gracefully
      ...
      let userCrewCount = 0;
      if (userId) {
        const { count } = await supabase.from('crews').select('*', {
          count: 'exact',
          head: true
      ...
      if (false && !skipUsageTracking) {
        ...
        try {
          ...
          if (!usageLimitData.allowed) {
            ...
            const upgradeMessage = usageLimitData.upgrade_message || "You've been organizing a lot this month! Upgrade for more fun: funlet.ai/upgrade";
            await sendSMS(phone_number, upgradeMessage, send_sms1, phone_number);
            return new Response(JSON.stringify({
              success: true,
      ...
      if (patternResult.action && userId && allowPatternMatching) {
        ...
        if (action === 'CREATE_CREW') {
          ...
          if (!crewName) {
            ...
            const { data: updateData, error: updateError } = await supabase.from('conversation_state').upsert({
              user_id: userId,
              phone_number: phone_number,
          ...
          try {
            ...
            const { data: recentEvents, error: eventsError } = await supabase.from('events').select('id, title, event_date, start_time, location, status').eq('creator_id', userId).eq('status', 'active').order('created_at', {
              ascending: false
            }).limit(5);
            if (eventsError) {
              ...
              if (eventName) {
                const matchingEvent = recentEvents.find((event)=>event.title.toLowerCase().includes(eventName.toLowerCase()) || eventName.toLowerCase().includes(event.title.toLowerCase()));
                if (matchingEvent) {
                  // Auto-select the matching event and show RSVP details
                  responseContent = await checkRSVPsForEvent(supabase, matchingEvent.id, userId, phone_number, send_sms1);
                  shouldSendSMS = true;
                    ...
                    extracted_data: [
                      {
                        action: 'CHECK_RSVPS',
                        substep: 1,
                        available_events: recentEvents.map((e)=>({
                            id: e.id,
                            title: e.title
                          }))
                      }
                ...
                if (recentEvents.length === 1) {
                  ...
                  responseContent = await checkRSVPsForEvent(supabase, onlyEvent.id, userId, phone_number, send_sms1);
                  shouldSendSMS = true;
                  // Clear conversation state
                  await supabase.from('conversation_state').update({
                    waiting_for: null,
          ...
          try {
            ...
            const { data: userEvents, error: eventsError } = await supabase.from('events').select('id, title, event_date, start_time, location, status').eq('creator_id', userId).eq('status', 'active').gte('event_date', new Date().toISOString().split('T')[0]).order('event_date', {
              ascending: true
            }).order('start_time', {
              ascending: true
            });
            if (eventsError) {
              ...
              responseContent = await showEventDetailsAndMenu(supabase, userId, phone_number, send_sms1, userEvents[0].id, phone_number);
              shouldSendSMS = true;
            } else if (eventName) {
              // Find all events matching the name (case-insensitive comparison)
              const matchingEvents = userEvents.filter((e)=>e.title.toLowerCase() === eventName.toLowerCase());
              if (matchingEvents.length === 1) {
                // Only one match - auto-select it
                responseContent = await showEventDetailsAndMenu(supabase, userId, phone_number, send_sms1, matchingEvents[0].id, phone_number);
                shouldSendSMS = true;
                  ...
                  extracted_data: [
                    {
                      action: 'MANAGE_EVENT',
                      current_page: page,
                      event_list: matchingEvents.map((e)=>({
                          id: e.id,
                          title: e.title,
          ...
          try {
            ...
            responseContent = await showEventDetailsAndMenu(supabase, userId, phone_number, send_sms1, selectedEvent.id, phone_number);
            return new Response(JSON.stringify({
              success: true,
              action: 'MANAGE_EVENT_SELECTION',
              response: responseContent,
          ...
          try {
            ...
            if (menuOption === 1) {
              ...
              responseContent = await showUninvitedCrewMembers(supabase, eventData, userId, phone_number, send_sms1);
              shouldSendSMS = true;
          ...
          try {
            ...
            const eventTitle = conversationState?.extracted_data?.[0]?.event_title;
            // Get current value
            const { data: eventData } = await supabase.from('events').select('title, event_date, start_time, end_time, location, notes').eq('id', eventId).single();
            let currentValue = '';
          ...
          try {
            ...
            if (field === 'date') {
              ...
              const { data: currentEventDataForTime } = await supabase.from('events').select('event_date').eq('id', eventIdForTime).single();
          ...
          try {
            ...
            if (editedFields.length === 0) {
              ...
              const menuDisplay = await showEventDetailsAndMenu(supabase, userId, phone_number, send_sms1, eventId, phone_number);
          ...
          try {
            ...
            const needsResend = editedFields.some((field)=>resendFields.includes(field));
            if (needsResend) {
          ...
          try {
            ...
            const { data: invitations } = await supabase.from('invitations').select('id, contact_id, contacts(phone_number, first_name)').eq('event_id', eventId);
          ...
          try {
            const eventId = conversationState?.extracted_data?.[0]?.event_id;
            // Show menu again
            responseContent = await showEventDetailsAndMenu(supabase, userId, phone_number, send_sms1, eventId, phone_number);
            shouldSendSMS = true;
            await sendSMS(phone_number, responseContent, send_sms1, phone_number);
            return new Response(JSON.stringify({
              success: true,
          ...
          try {
            ...
            const { data: sourceEvent, error: sourceError } = await supabase.from('events').select('*').eq('id', sourceEventId).single();
          ...
          try {
            ...
            const { data: newEvent, error: createError } = await supabase.from('events').insert({
              title: newEventName,
            ...
            if (!newEvent.crew_id) {
              ...
              if (invitationsProcessed) {
                ...
                const successfulInvites = inviteResult.results ? inviteResult.results.filter((r)=>r.status === 'success' || r.status === 'sent').length : 0;
          ...
          try {
            ...
            const { data: invitations } = await supabase.from('invitations').select('id').eq('event_id', eventId);
            if (invitations && invitations.length > 0) {
              ...
              const { error: deleteError } = await supabase.from('events').update({
                status: 'cancelled'
              }).eq('id', eventId);
              if (deleteError) {
              ...
              for (const invitation of invitations){
                if (invitation.contact_id && invitation.contacts && invitation.contacts.phone_number) {
                  try {
                    ...
                    const personalizedMessage = contactName !== 'there' ? `Hi ${contactName}, ${messageToSend}` : messageToSend;
                    await sendSMS(contactPhone, personalizedMessage, send_sms1, phone_number);
                    sentCount++;
                  } catch (error) {
          ...
          try {
            ...
            if (invitations && invitations.length > 0) {
              ...
              const messageToSend = cancellationMessage && cancellationMessage.trim() !== 'skip' ? cancellationMessage.trim() : `"${eventTitle}" has been canceled.`;
              // Send cancellation messages to all invitees
              for (const invitation of invitations){
                if (invitation.contact_id && invitation.contacts && invitation.contacts.phone_number) {
                  try {
          ...
          try {
            ...
            if (!crewList || crewIndex < 0 || crewIndex >= crewList.length) {
              ...
              const selectedCrew = crewList[crewIndex];
              // Get crew members
              const { data: crewMembers, error: membersError } = await supabase.from('crew_members').select(`
                       contacts (
                         first_name,
                         phone_number
                       )
              ...
              if (membersError) {
                ...
                const memberNames = crewMembers.slice(0, displayLimit).map((member)=>member.contacts.first_name).join(', ');
                // Format message based on crew size
                const memberDisplay = crewMembers.length > displayLimit ? `${selectedCrew.name}: ${memberNames}... (${crewMembers.length} total)` : `${selectedCrew.name} (${crewMembers.length}): ${memberNames}`;
          ...
          try {
            ...
            const { data: userCrews, error: crewsError } = await supabase.from('crews').select('id, name').eq('creator_id', userId).order('name');
            if (crewsError) {
            ...
            if (!userCrews || userCrews.length === 0) {
              ...
              responseContent = await showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, crew.id, crew.name, phone_number);
              return new Response(JSON.stringify({
                success: true,
                action: 'CHECK_CREW_MEMBERS',
                response: responseContent,
              ...
              if (crewName) {
                ...
                const partialMatches = userCrews.filter((c)=>c.name.toLowerCase().includes(crewName.toLowerCase()) && !exactMatches.some((e)=>e.id === c.id));
                ...
                if (allMatches.length === 0) {
                  ...
                  if (ownership.isValid) {
                    responseContent = await showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, selectedCrew.id, selectedCrew.name, phone_number);
                    return new Response(JSON.stringify({
                      success: true,
                      action: 'CHECK_CREW_MEMBERS',
                      response: responseContent,
          ...
          try {
            ...
            if (menuOption === 1) {
              ...
              if (memberList.length === 0) {
                ...
                await sendSMS(phone_number, responseContent, send_sms1, phone_number);
                // Re-show menu
                responseContent = await showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, crewId, crewName, phone_number);
                return new Response(JSON.stringify({
                  success: true,
                  action: 'CREW_MANAGEMENT_MENU_SELECTION',
                  response: responseContent,
              ...
              shouldSendSMS = true;
              await sendSMS(phone_number, linkMessage, send_sms1, phone_number);
              // Re-show menu
              const menuDisplay = await showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, crewId, crewName, phone_number);
              ...
              shouldSendSMS = true;
              await sendSMS(phone_number, qrMessage, send_sms1, phone_number);
              // Re-show menu
              const menuDisplay = await showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, crewId, crewName, phone_number);
          ...
          try {
            ...
            const { count: contactCount, error: countError } = await supabase.from('contacts').select('id', {
              count: 'exact',
              head: true
            }).eq('user_id', userId);
            ...
            if (searchQuery) {
              ...
              if (contacts.length === 1) {
                responseContent = await showContactActionsMenu(supabase, userId, phone_number, send_sms1, contacts[0].id, phone_number);
                return new Response(JSON.stringify({
                  success: true,
                  action: 'EDIT_CONTACT',
                  response: responseContent
          ...
          try {
            ...
            responseContent = await showContactActionsMenu(supabase, userId, phone_number, send_sms1, contactId, phone_number);
            return new Response(JSON.stringify({
              success: true,
              action: action,
              response: responseContent
          ...
          try {
            ...
            const { data: existingContact } = await supabase.from('contacts').select('id').eq('user_id', userId).eq('phone_number', formattedPhone).neq('id', contactId).maybeSingle();
            if (existingContact) {
          ...
          try {
            ...
            const { data: contactOwnershipData, error: contactCheckError } = await supabase.from('contacts').select('id, user_id').eq('id', contactId).eq('user_id', userId).single();
            ...
            await supabase.from('crew_members').delete().eq('contact_id', contactId);
            // Delete from invitations (cascade)
            await supabase.from('invitations').delete().eq('contact_id', contactId);
            // Delete contact
            const { error: deleteError } = await supabase.from('contacts').delete().eq('id', contactId).eq('user_id', userId);
          ...
          try {
            ...
            const { error: removeError } = await supabase.from('crew_members').delete().in('id', memberIdsToRemove).eq('crew_id', crewId);
            if (removeError) {
            ...
            const updatedDisplay = await showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, crewId, crewName, phone_number);
          ...
          try {
            ...
            if (newCrewName.toLowerCase() === oldCrewName.toLowerCase()) {
              ...
              const menuDisplay = await showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, crewId, oldCrewName, phone_number);
            ...
            const { error: updateError } = await supabase.from('crews').update({
              name: newCrewName
            }).eq('id', crewId).eq('creator_id', userId);
            if (updateError) {
            ...
            // Re-show updated member list and menu
            const updatedDisplay = await showCrewMembersAndMenu(supabase, userId, phone_number, send_sms1, crewId, newCrewName, phone_number);
          ...
          try {
            ...
            const { error: deleteError } = await supabase.from('crews').delete().eq('id', crewId).eq('creator_id', userId);
            if (deleteError) {
          ...
          try {
            ...
            if (!eventList || eventIndex < 0 || eventIndex >= eventList.length) {
              ...
              if (cameFromCheckRsvps) {
                ...
                responseContent = await checkRSVPsForEvent(supabase, selectedEvent.id, userId, phone_number, send_sms1);
                shouldSendSMS = true;
              } else {
                // Default behavior
                await supabase.from('conversation_state').update({
        ...
        // EVENT_DETAILS_INPUT handler removed - using progressive workflow only
            ...
            if (!crewId) {
              ...
              try {
                ...
                if (eventError) {
                  ...
                  const { data: crewMembers } = await supabase.from('crew_members').select('id').eq('crew_id', crewId);
          ...
          try {
            ...
            const { data: userEvents } = await supabase.from('events').select(`
                     ...
                     location,
                     crews (name)
                   `).eq('creator_id', userId).eq('status', 'active').gte('event_date', new Date().toISOString().split('T')[0]).order('event_date', {
              ascending: true
            });
            if (!userEvents || userEvents.length === 0) {
              ...
              // If event name was provided, try to find a matching event
              if (eventName) {
                const matchingEvent = userEvents.find((event)=>event.title.toLowerCase().includes(eventName.toLowerCase()) || eventName.toLowerCase().includes(event.title.toLowerCase()));
                if (matchingEvent) {
                  // Auto-select the matching event and show uninvited crew members
                  responseContent = await showUninvitedCrewMembers(supabase, matchingEvent, userId, phone_number, send_sms1);
                  shouldSendSMS = true;
                    ...
                    extracted_data: [
                      {
                        action: 'INVITE_MORE_PEOPLE',
                        substep: 1,
                        available_events: userEvents.map((e)=>({
                            id: e.id,
                            title: e.title
                          }))
                      }
                ...
                responseContent = await showUninvitedCrewMembers(supabase, event, userId, phone_number, send_sms1);
                shouldSendSMS = true;
                // Set appropriate waiting state based on the response
                let waitingFor = null;
                if (responseContent.includes("haven't been invited yet")) {
          ...
          try {
            if (extractedData.event_index !== undefined) {
              ...
              const { data: conversationStateDataData } = await supabase.from('conversation_state').select('extracted_data').eq('user_id', userId).single();
              const eventList = conversationStateDataData?.extracted_data?.find((item)=>item.action === 'INVITE_MORE_PEOPLE')?.available_events;
          ...
          try {
            ...
            if (extractedData.invite_method === 'existing_crew') {
              ...
              if (existingData.length > 0) {
                for(let i = existingData.length - 1; i >= 0; i--){
                  ...
                  if (item.action === 'INVITE_MORE_PEOPLE_STEP_2' && item.event_id) {
                    ...
                    const { data: eventData } = await supabase.from('events').select('crew_id').eq('id', item.event_id).single();
              ...
              let crewQuery = supabase.from('crews').select('id, name').eq('creator_id', userId).order('name');
              ...
              if (userCrews && userCrews.length > 0) {
                ...
                const updatedExtractedDataWithCrewList = [
                ...
                // Update conversation state
                const { error: updateError } = await supabase.from('conversation_state').update({
          ...
          try {
            ...
            const { data: conversationStateDataData } = await supabase.from('conversation_state').select('extracted_data').eq('user_id', userId).single();
            const crewList = conversationStateDataData?.extracted_data?.find((item)=>item.action === 'INVITE_MORE_PEOPLE_CREW_LIST_SHOWN')?.crew_list;
          ...
          try {
            ...
            if (extractedMembers.length > 0) {
              ...
              if (eventId) {
                const { data: eventData } = await supabase.from('events').select('crew_id, crews (name)').eq('id', eventId).single();
                if (eventData) {
              ...
              if (!crewId && !eventId) {
                ...
                const { data: recentEvent } = await supabase.from('events').select('id, title, crew_id, crews (name)').eq('creator_id', userId).eq('status', 'active').order('created_at', {
                  ascending: false
                }).limit(1).single();
                if (recentEvent) {
                  ...
                  console.log('MEMBER_INPUT_FOR_EVENT: Found crew via database fallback', {
                    crewId,
                    crewName,
                    eventId
                  });
              ...
              if (!crewId) {
                ...
                for (const member of extractedMembers){
                  ...
                  // Check if contact already exists
                  const { data: existingContact } = await supabase.from('contacts').select('id, first_name, last_name').eq('user_id', userId).eq('phone_number', member.phone).single();
                  if (existingContact) {
                ...
                if (processedContacts.length === extractedMembers.length) {
                  ...
                  const { error: crewMemberError } = await supabase.from('crew_members').insert(crewMemberInserts);
                  if (crewMemberError) {
          ...
          try {
            ...
            if (eventId && crewId && crewMembers.length > 0) {
              try {
                ...
                const { data: eventData } = await supabase.from('events').select('crew_id').eq('id', eventId).single();
                if (eventData?.crew_id) {
          ...
          try {
            ...
            if (!eventId || contacts.length === 0) {
              ...
              try {
                ...
                const { data: eventCrews } = await supabase.from('crews').select('id, name').eq('creator_id', userId).limit(1);
                ...
                if (!eventCrewId) {
                  ...
                  for (const contact of contacts){
                    ...
                    // Check if contact already exists
                    const { data: existingContact } = await supabase.from('contacts').select('id').eq('phone_number', contact.phone).eq('user_id', userId).single();
                    let contactId;
                    if (existingContact) {
                    ...
                    if (contactId && eventCrewId) {
                      const { error: crewMemberError } = await supabase.from('crew_members').insert({
                        crew_id: eventCrewId,
                        contact_id: contactId
                      });
                      if (crewMemberError) {
                  ...
                  if (contactIds.length > 0) {
                    const { data: crewMembers } = await supabase.from('crew_members').select('id').eq('crew_id', eventCrewId).in('contact_id', contactIds);
          ...
          try {
            ...
            const { data: freshConversationState } = await supabase.from('conversation_state').select('extracted_data').eq('user_id', userId).single();
          ...
          try {
            ...
            if (crewsError) {
              ...
              const requestedCrew = userCrews.find((crew)=>crew.name.toLowerCase() === requestedCrewName.toLowerCase());
              if (requestedCrew) {
                ...
                extracted_data: [
                  {
                    action: 'SEND_INVITATIONS',
                    substep: 1,
                    available_crews: userCrews.map((c)=>({
                        id: c.id,
                        name: c.name
                      })),
          ...
          try {
            ...
            if (extractedData.crewName) {
              ...
              if (!crewsError && userCrews) {
                ...
                targetCrew = userCrews.find((crew)=>crew.name.toLowerCase() === extractedData.crewName.toLowerCase());
                if (targetCrew) {
            ...
            if (targetCrew) {
              ...
              shouldSendSMS = true;
              // Update conversation state for progressive workflow
              const { error: upsertError } = await supabase.from('conversation_state').upsert({
                user_id: userId,
        ...
        // INVALID_EVENT_DETAILS_INPUT handler removed - using progressive workflow only
          ...
          try {
            ...
            const eventListData = conversationState?.extracted_data?.find((item)=>item.action === 'SYNC_UP_EVENT_LIST_SHOWN');
            if (!eventListData || !eventListData.event_list) {
              ...
              if (!selectedEvent) {
                ...
                // Get full event details
                const { data: eventData } = await supabase.from('events').select('*').eq('id', selectedEvent.id).single();
                if (eventData) {
                  const eventDate = new Date(`${eventData.event_date}T${eventData.start_time || '00:00:00'}`);
                  const formattedDate = eventDate.toLocaleDateString('en-US', {
                    weekday: 'short',
          ...
          try {
            ...
            const syncUpListData = conversationState?.extracted_data?.find((item)=>item.action === 'RE_SYNC_SYNC_UP_LIST_SHOWN');
          ...
          try {
            ...
            if (!reSyncData) {
              ...
              const { error: deleteOptionsError } = await supabase.from('sync_up_options').delete().eq('sync_up_id', reSyncData.sync_up_id);
              if (deleteOptionsError) {
                ...
                const { error: optionsError } = await supabase.from('sync_up_options').insert(syncUpOptions);
                if (optionsError) {
                  ...
                  const { error: deleteResponsesError } = await supabase.from('sync_up_responses').delete().eq('sync_up_id', reSyncData.sync_up_id);
                  if (deleteResponsesError) {
                    console.error('Error deleting old sync up responses:', deleteResponsesError);
                  }
                  // Get crew members for the sync up's crew
                  const { data: crewMembers } = await supabase.from('crew_members').select('contact_id, contacts(phone_number, first_name)').eq('crew_id', reSyncData.crew_id);
                  if (crewMembers && crewMembers.length > 0) {
                      ...
                      if (member.contacts?.phone_number) {
                        ...
                        if (result.success) {
                          ...
                          if (!actualContactId) {
                            ...
                            const plusVariations = phoneVariations.map((phone)=>'+' + phone);
                            phoneVariations.push(...plusVariations);
                            const { data: contactRows } = await supabase.from('contacts').select('id').in('phone_number', phoneVariations);
                          ...
                          await supabase.from('conversation_state').delete().eq('phone_number', member.contacts.phone_number);
                          const { data: convStateData, error: convStateError } = await supabase.from('conversation_state').insert({
                          ...
                          if (actualContactId) {
                            ...
                            const { data: responseData, error: responseError } = await supabase.from('sync_up_responses').upsert({
                              contact_id: actualContactId,
                              sync_up_id: reSyncData.sync_up_id,
                              option_ids: [],
          ...
          try {
            ...
            if (!syncUpData) {
              ...
              if (eventError || !newEvent) {
                ...
                const { data: syncUpRecord, error: syncUpError } = await supabase.from('sync_ups').insert({
                  creator_id: userId,
                  event_id: newEvent.id,
                  crew_id: syncUpData.crew_id,
                ...
                if (syncUpError) {
                  ...
                  if (optionsError) {
                    ...
                    const { data: crewMembers } = await supabase.from('crew_members').select('contact_id, contacts(phone_number, first_name)').eq('crew_id', syncUpData.crew_id);
                    if (crewMembers && crewMembers.length > 0) {
                      // Get host name from profile
                      let hostName = syncUpData.crew_name; // Fallback to crew name
                      try {
                        const { data: profileData } = await supabase.from('profiles').select('first_name, last_name').eq('id', userId).single();
                        if (profileData) {
                          hostName = profileData.last_name ? `${profileData.first_name} ${profileData.last_name}` : profileData.first_name || syncUpData.crew_name;
                        }
                      } catch (err) {
                      ...
                      const formatTimeOptionSimple = (startTime, endTime)=>{
        ...
        // Progressive SYNC_UP workflow handlers
          ...
          const { data: syncUps, error: syncUpsError } = await supabase.from('sync_ups').select(`
                   id,
                   name,
                   location,
                   created_at,
                   ...
                   crews (id, name)
                 `).eq('events.creator_id', userId).eq('status', 'sent').order('created_at', {
            ascending: false
          });
          ...
          if (eventName) {
            const matchingSyncUp = syncUps.find((syncUp)=>syncUp.name.toLowerCase().includes(eventName.toLowerCase()) || syncUp.events.title.toLowerCase().includes(eventName.toLowerCase()));
            if (matchingSyncUp) {
          ...
          for(let index = 0; index < syncUps.length; index++){
            ...
            const { data: syncUpOptions } = await supabase.from('sync_up_options').select('*').eq('sync_up_id', syncUp.id).neq('idx', 0) // Exclude global None option
            .order('idx', {
              ascending: true
            });
            ...
            let timeOptionsText = '';
            if (syncUpOptions && syncUpOptions.length > 0) {
              const formattedOptions = syncUpOptions.map((opt)=>{
                const { dayMonth, timeText } = formatTimeRangeForOptionGlobal(opt.start_time, opt.end_time);
          ...
          if (conversationState?.waiting_for === 'send_message_context') {
            ...
            if (extractedData.context === 'sync_up') {
              ...
              const { data: syncUps, error: syncUpsError } = await supabase.from('sync_ups').select(`
                    ...
                    crew_id,
                    crews(name)
                  `).eq('creator_id', userId).order('created_at', {
                ascending: false
              }).limit(10);
              if (syncUpsError || !syncUps || syncUps.length === 0) {
                  ...
                  extracted_data: [
                    {
                      ...
                      substep: 2,
                      context: 'sync_up',
                      available_sync_ups: syncUps.map((s)=>({
                          id: s.id,
                          name: s.name,
                          location: s.location,
                          crew_id: s.crew_id,
              ...
              // Show event list
              const { data: events, error: eventsError } = await supabase.from('events').select('id, title, event_date, start_time, location').eq('creator_id', userId).gte('event_date', new Date().toISOString().split('T')[0]).order('event_date', {
                ascending: true
              });
            ...
            // Step 3: Handle event selection
            if (extractedData.event_index !== undefined) {
              const { data: events } = await supabase.from('events').select('id, title, event_date, start_time, location').eq('creator_id', userId).gte('event_date', new Date().toISOString().split('T')[0]).order('event_date', {
                ascending: true
              });
            ...
            if (extractedData.sync_up_index !== undefined) {
              const { data: currentState } = await supabase.from('conversation_state').select('extracted_data').eq('user_id', userId).single();
            ...
            if (extractedData.targeting_index !== undefined) {
              ...
              if (!targetingGroup) {
                ...
                if (targetingGroup === 'everyone') {
                  ...
                  filteredInvitations = validInvitations.filter((inv)=>inv.status === 'sent' || inv.status === 'failed' && inv.response_note === 'no_response');
                } else if (targetingGroup === 'coming') {
                  filteredInvitations = validInvitations.filter((inv)=>inv.response_note === 'in' || inv.response_note === 'yes' || inv.response_note === '1');
                } else if (targetingGroup === 'maybe') {
                  filteredInvitations = validInvitations.filter((inv)=>inv.response_note === 'maybe' || inv.response_note === '3');
                } else if (targetingGroup === 'out') {
                  filteredInvitations = validInvitations.filter((inv)=>inv.response_note === 'out' || inv.response_note === 'no' || inv.response_note === '2');
                }
                if (filteredInvitations.length === 0) {
                  // Friendly, group-specific empty-state messages
                  const emptyGroupMessageMap = {
            ...
            if (extractedData.confirm === true) {
              ...
              if (messageText && filteredInvitations.length > 0) {
                ...
                for (const inv of filteredInvitations){
                  ...
                  if (!phone) continue;
                  try {
                    await sendSMS(phone, messageText, send_sms1, phone_number);
                    successCount++;
                  } catch (error) {
            ...
            if (extractedData.confirm === true) {
              ...
              if (messageText && recipientList.length > 0) {
                ...
                for (const recipient of recipientList){
                  try {
                    await sendSMS(recipient.phone_number, messageText, send_sms1, phone_number);
                    successCount++;
                  } catch (error) {
            ...
            const { data: verifyState } = await supabase.from('conversation_state').select('waiting_for, current_state').eq('user_id', userId).single();
          ...
          try {
            ...
            if (!eventId) {
              const { data: recentEvent } = await supabase.from('events').select(`
                      id,
                      title,
                      event_date,
                      start_time,
                      crews (name)
                    `).eq('creator_id', userId).eq('status', 'active').order('created_at', {
                ascending: false
              }).limit(1).single();
              if (recentEvent) {
                eventId = recentEvent.id;
            ...
            if (!eventId) {
              ...
              const { data: eventData } = await supabase.from('events').select('crew_id, event_date, start_time, title, shorten_event_url').eq('id', eventId).single();
              if (eventData) {
              ...
              if (eventData?.crew_id) {
                ...
                const { data: invitedMembers } = await supabase.from('invitations').select('contact_id').eq('event_id', eventId);
                const invitedContactIds = new Set(invitedMembers?.map((inv)=>inv.contact_id) || []);
                // Filter uninvited members
                const uninvitedMembers = crewMembers.filter((member)=>member.contacts && !invitedContactIds.has(member.contacts.id)).map((member)=>member.contacts);
                if (uninvitedMembers.length === 0) {
                  ...
                  const { error: insertError } = await supabase.from('invitations').insert(invitations);
                  if (insertError) {
          ...
          try {
            ...
            if (eventId) {
              ...
              const { data: currentStateData } = await supabase.from('conversation_state').select('extracted_data').eq('user_id', userId).single();
          ...
          try {
            ...
            if (conversationStateData?.extracted_data && Array.isArray(conversationStateData.extracted_data)) {
              for(let i = conversationStateData.extracted_data.length - 1; i >= 0; i--){
                ...
                if (item.action === 'ADD_MEMBERS_TO_EVENT' || item.action === 'INVITE_MORE_PEOPLE_STEP_2') {
                  ...
                  crewId = item.crew_id;
                  crewName = item.crew_name;
                  console.log('Found event/crew info:', {
                    eventId,
                    crewId,
                    crewName
                  });
            ...
            if (eventId && !crewId) {
              ...
              if (eventData) {
                ...
                console.log('Looked up crew info from event:', {
                  crewId,
                  crewName
                });
              }
            }
            console.log('Final event/crew info:', {
              eventId,
              crewId,
              crewName
            });
            if (!eventId || !crewId) {
              ...
              // Get event details
              const { data: eventData } = await supabase.from('events').select('title, event_date, start_time, shorten_event_url').eq('id', eventId).single();
              if (!eventData) {
                responseContent = "I didn't understand that. Reply with an event number or 'exit' to do something else.";
                shouldSendSMS = true;
              } else {
                ...
                if (uninvitedMembers.length === 0) {
                  ...
                  if (insertError) {
                    ...
                    const eventTime = eventData.start_time ? new Date(`2000-01-01T${eventData.start_time}`).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit'
                    }) : '';
                    // Fetch event with shorten_event_url (with retry logic)
...
Reply with a command or contact support@funlet.ai with any questions.`;
          ...
          const { data: syncUps, error: syncUpsError } = await supabase.from('sync_ups').select(`
                 ...
                 `).eq('events.creator_id', userId).order('created_at', {
            ascending: false
          });
          if (syncUpsError || !syncUps || syncUps.length === 0) {
            responseContent = 'No active sync ups found. Type \'Sync Up\' to create one or \'exit\' to do something else.';
          ...
          let selectedSyncUp = null;
          if (eventName) {
            selectedSyncUp = syncUps.find((su)=>su.events.title.toLowerCase().includes(eventName.toLowerCase()));
            if (!selectedSyncUp) {
          ...
          const { data: syncUp } = await supabase.from('sync_ups').select(`
                  id,
                  event_id,
                  crew_id,
          ...
          const { data: options } = await supabase.from('sync_up_options').select('*').eq('sync_up_id', syncStatusData.sync_up_id).order('idx');
          ...
          const { count: memberCount } = await supabase.from('crew_members').select('*', {
            count: 'exact',
            head: true
          ...
          const { data: latestState } = await supabase.from('conversation_state').select('extracted_data').eq('user_id', userId).maybeSingle();
          ...
          const { data: newEvent, error: createEventError } = await supabase.from('events').insert({
            title: optionData.sync_up_name || 'Sync Up Event',
            location: optionData.location || 'TBD',
            start_time: startTimeStr,
            end_time: endTimeStr,
          ...
          try {
            ...
            if (!crewId && extractedData.crewName) {
              ...
              if (extractedData.crewName.toLowerCase() === 'my crew') {
                ...
                const { data: userCrews, error: crewsError } = await supabase.from('crews').select('id, name').eq('creator_id', userId).ilike('name', extractedData.crewName);
                if (crewsError) {
            ...
            if (crewId) {
              ...
              if (extractedData.crew_members && extractedData.crew_members.length > 0) {
                ...
                for (const member of members){
                  // Check if contact already exists
                  const { data: existingContact } = await supabase.from('contacts').select('id').eq('phone_number', member.phone).eq('user_id', userId).single();
                  let contactId;
                  if (existingContact) {
                    ...
                    if (contactError) {
                      ...
                      if (contactError.code === '23505') {
                        const { data: duplicateContact } = await supabase.from('contacts').select('id').eq('phone_number', member.phone).eq('user_id', userId).single();
                        if (duplicateContact) {
                  ...
                  const { data: existingMember } = await supabase.from('crew_members').select('id').eq('crew_id', crewId).eq('contact_id', contactId).single();
                  if (existingMember) {
              ...
              const { data: userCrews } = await supabase.from('crews').select('id, name').eq('creator_id', userId).order('name');
              if (userCrews && userCrews.length === 0) {
          ...
          try {
            ...
            const { count: totalCount } = await supabase.from('contacts').select('*', {
              count: 'exact',
              head: true
            }).eq('user_id', userId).or(`first_name.ilike.%${searchName}%,last_name.ilike.%${searchName}%`);
            // Search contacts by name - get first 10
            const { data: contacts, error: searchError } = await supabase.from('contacts').select('id, first_name, last_name, phone_number').eq('user_id', userId).or(`first_name.ilike.%${searchName}%,last_name.ilike.%${searchName}%`).order('created_at', {
              ascending: false
            }).limit(10);
            if (searchError) {
            ...
            if (!contacts || contacts.length === 0) {
              ...
              const cleanDigits = digits.startsWith('1') && digits.length === 11 ? digits.substring(1) : digits;
              const formattedPhone = cleanDigits.length === 10 ? `${cleanDigits.substring(0, 3)}-${cleanDigits.substring(3, 6)}-${cleanDigits.substring(6)}` : contact.phone_number;
          ...
          try {
            ...
            if (existingMember) {
              ...
              const { data: contactData } = await supabase.from('contacts').select('phone_number').eq('id', contactId).eq('user_id', userId).single();
          ...
          try {
            ...
            // Check if contact is already in crew
            const { data: existingMember } = await supabase.from('crew_members').select('id').eq('crew_id', crewId).eq('contact_id', selectedContact.id).single();
            if (existingMember) {
              const joinLink = await getCrewJoinLink(supabase, crewId);
              // Format phone number as XXX-XXX-XXXX
              const digits = selectedContact.phone?.replace(/\D/g, '') || '';
              const cleanDigits = digits.startsWith('1') && digits.length === 11 ? digits.substring(1) : digits;
              const formattedPhone = cleanDigits.length === 10 ? `${cleanDigits.substring(0, 3)}-${cleanDigits.substring(3, 6)}-${cleanDigits.substring(6)}` : selectedContact.phone || '';
      ...
      const { data: constantData, error: constantError } = await supabase.from('constants').select('value').eq('key', 'assistant_id_v2').single();
      ...
      if (phone_number) {
        ...
        const { data, error } = await supabase.from('profiles').select('id, subscription_status, is_onboarded, phone_number').in('phone_number', phoneVariations).limit(1);
        ...
        if (existingProfile) {
          ...
          if (existingProfile.is_onboarded === false) {
            ...
            const { data: conversationHistory } = await supabase.from('conversation_state').select('id, last_action').eq('phone_number', foundPhoneVariation).order('created_at', {
              ascending: false
            }).limit(1);
        ...
        if (!isFirstTimeUser && foundPhoneVariation && existingProfile && existingProfile.is_onboarded !== true) {
          const { data: anyConversationHistory } = await supabase.from('conversation_state').select('id').eq('phone_number', foundPhoneVariation).limit(1);
      ...
      if (phone_number) {
        ...
        let conversationState = null;
        const { data } = await supabase.from('conversation_state').select('thread_id, thread_created_at, expires_at, phone_number, current_state, waiting_for, extracted_data').in('phone_number', phoneVariations).order('created_at', {
          ascending: false
        }).limit(1);
        if (data && data.length > 0) {
      ...
      const { data: resetState } = await supabase.from('conversation_state').select('waiting_for').eq('user_id', userId).single();
      if (resetState?.waiting_for === 'reset_confirmation') {
        if (cleanMessage === 'yes') {
          ...
          const { data: stateData } = await supabase.from('conversation_state').select('thread_id').eq('user_id', userId).single();
          ...
          const { data: newState, error: insertError } = await supabase.from('conversation_state').insert({
            user_id: userId,
            phone_number: phone_number,
          ...
          // Send SMS response
          if (phone_number) {
            const smsResult = await sendSMS(phone_number, resetResponse, send_sms1, phone_number);
          ...
          // Send SMS response
          if (phone_number) {
            const smsResult = await sendSMS(phone_number, cancelResponse, send_sms1, phone_number);
      ...
      if (runStatus === 'requires_action') {
        ...
        if (runDetailsResponse.ok) {
          ...
          const submitResponse = await fetch(`https://api.openai.com/v1/threads/${threadId}/runs/${runId}/submit_tool_outputs`, {
            ...
            body: JSON.stringify({
              tool_outputs: runDetails.required_action.submit_tool_outputs.tool_calls.map((toolCall)=>({
      ...
      if (action === 'UNKNOWN_MESSAGE') {
        ...
        if (conversationStateData?.extracted_data && conversationStateData.extracted_data.length > 0 && conversationStateData.extracted_data[conversationStateData.extracted_data.length - 1]?.action === 'CREW_MEMBERS_SHOWN') {
        ...
        if (conversationState?.waiting_for === 'post_crew_members_view') {
          ...
          if (phone_number && userId) {
            const { data: conversationStateDataData } = await supabase.from('conversation_state').select('current_state, extracted_data').eq('user_id', userId).single();
      ...
      console.log('Final SMS check: shouldSendSMS =', shouldSendSMS, 'send_sms =', send_sms1, 'phone_number =', phone_number, 'responseContent =', responseContent?.substring(0, 100) + '...');
      if (shouldSendSMS && phone_number && responseContent) {
        const smsResult = await sendSMS(phone_number, responseContent, send_sms1, phone_number);
        console.log('SMS result:', smsResult);
        // Handle SMS limit exceeded
        if (smsResult && smsResult.success === false && smsResult.error === 'SMS_LIMIT_EXCEEDED') {
          const upgradeMessage = smsResult.limit_data?.upgrade_message || "You're really making the most of Funlet! Upgrade for more fun: funlet.ai/upgrade";
          return new Response(JSON.stringify({
            success: true,
      ...
      const { data: contactRows, error: contactError } = await supabase.from('contacts').select('id, phone_number').in('phone_number', phoneVariations);
      ...
      const { data: csRows, error: csErr } = await supabase.from('conversation_state').select('user_id, waiting_for, extracted_data').in('phone_number', phoneVariations).order('created_at', {
        ascending: false
      }).limit(1);
      const conversationState = csRows?.[0] || null;
      console.log(`📋 Conversation state query result:`, {
        conversationState,
        csErr
      });
      ...
      if (contactId) {
        ...
        if (!syncUpId) {
          ...
          const { data: activeSyncUpResponse } = await supabase.from('sync_up_responses').select('sync_up_id, sync_ups!inner(id, status)').eq('contact_id', contactId).in('sync_ups.status', [
            'active',
            'sent'
          ]).order('created_at', {
            ascending: false
      ...
      if (syncUpId) {
        ...
        const isValidSyncUpReply = /^none(?:\s+of\s+(these|those|them))?$/.test(lowerMsg) || lowerMsg.trim() === '0' || /^[123\s,]+$/.test(lowerMsg);
        ...
        const { data: syncUpResponse, error: responseError } = await supabase.from('sync_up_responses').select(`
          sync_up_id, 
          option_ids, 
          response_type,
          sync_ups!inner(status)
        `).eq('contact_id', contactId).eq('sync_up_id', syncUpId).in('sync_ups.status', [
          'active',
          'sent'
        ]).maybeSingle();
        ...
        if (!syncUpResponse && contactId && syncUpId) {
          ...
          const { error: createResponseError } = await supabase.from('sync_up_responses').insert({
            contact_id: contactId,
            sync_up_id: syncUpId,
            option_ids: [],
            response_type: 'selected'
        ...
        console.log(`📝 sync_up_responses lookup:`, {
          syncUpResponse,
          responseError,
          contactId,
          syncUpId
        ...
        if (syncUpId) {
          ...
          // Get sync up options
          const { data: optionRows } = await supabase.from('sync_up_options').select('id, idx, option_text').eq('sync_up_id', syncUpId).neq('idx', 0) // Exclude global None option
          .order('idx');
          if (optionRows && optionRows.length > 0) {
            ...
            if (!isNone) {
              ...
              try {
                const { data: noneOption } = await supabase.from('sync_up_options').select('id').eq('idx', 0).maybeSingle();
                if (noneOption?.id) {
            ...
            await supabase.from('sync_up_responses').upsert({
              contact_id: contactId,
              sync_up_id: syncUpId,
      ...
      if (conversationState?.waiting_for === 'event_select') {
        ...
        const eventListItem = (conversationState.extracted_data || []).find((x)=>x?.action === 'RSVP_EVENT_LIST');
        ...
        if (!numericMatch || eventList.length === 0) {
          ...
          await sendSMS(phone_number, eventListMsg, send_sms1, phone_number);
          return new Response(JSON.stringify({
            success: true,
            action: 'RSVP_EVENT_SELECT_PROMPT',
        ...
        if (idx < 0 || idx >= eventList.length) {
          ...
          return new Response(JSON.stringify({
            success: true,
            action: 'RSVP_EVENT_SELECT_INVALID',
            message: eventListMsg
          }), {
            headers: {
              ...corsHeaders,
        ...
        try {
          ...
          const { data: updateResult, error: updateError } = await supabase.from('conversation_state').update({
            waiting_for: 'rsvp',
            current_state: 'rsvp_flow',
            extracted_data: [
              ...(conversationState?.extracted_data || []).filter((x)=>x?.action !== 'RSVP_SELECTED_INVITATION'),
              {
                action: 'RSVP_SELECTED_INVITATION',
                invitation_id: selected.id
              }
            ]
          }).eq('phone_number', normalizedPhone).select('id');
          console.log('DEBUG: RSVP update result:', {
            updateResult,
            updateError
          });
          if (updateError) {
        ...
        await sendSMS(phone_number, prompt, send_sms1, phone_number);
        return new Response(JSON.stringify({
          success: true,
          action: 'RSVP_CHOICE_PROMPT',
          message: prompt
        }), {
          headers: {
            ...corsHeaders,
      ...
      if (isRsvpResponse) {
        ...
        if (conversationState?.waiting_for === 'rsvp') {
          const selItem = (conversationState.extracted_data || []).find((x)=>x?.action === 'RSVP_SELECTED_INVITATION');
          const selectedInvitationId = selItem?.invitation_id;
          if (selectedInvitationId) {
            const { data: inv, error: invLoadErr } = await supabase.from('invitations').select('id, event_id, status, events!inner(title, creator_id, shorten_calendar_url)').eq('id', selectedInvitationId).single();
            if (!invLoadErr && inv) {
              ...
              if (!updateError) {
                ...
                const confirmationMsg = rsvpStatus === 'in' ? `Great! You're in for ${inv.events.title}! 🎉${calendarLink}` : rsvpStatus === 'out' ? `Got it, you're out for ${inv.events.title}. Maybe next time!` : `Noted! You're a maybe for ${inv.events.title}. Let us know when you decide!${calendarLink}`;
                await sendSMS(phone_number, confirmationMsg, send_sms1, phone_number);
                return new Response(JSON.stringify({
                  success: true,
        ...
        const { data: invitations, error: invError } = await supabase.from('invitations').select('id, event_id, status, events!inner(title, creator_id, shorten_calendar_url)').eq('contact_id', contactId).eq('events.status', 'active').eq('response_note', 'no_response').order('created_at', {
          ascending: false
        });
        if (invError) {
        ...
        if (invitations.length > 1) {
          const eventList = invitations.map((inv)=>({
              id: inv.id,
          ...
          try {
            ...
            const { data: existingState } = await supabase.from('conversation_state').select('id').eq('phone_number', normalizedPhone).limit(1);
            if (!existingState || existingState.length === 0) {
              console.log('DEBUG: No existing state found, inserting new conversation state');
              const { data: insertResult, error: insertError } = await supabase.from('conversation_state').insert({
                user_id: null,
          ...
          return new Response(JSON.stringify({
            ...
            action: 'RSVP_EVENT_SELECT_PROMPT',
            message: eventListMsg,
            event_count: invitations.length
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
