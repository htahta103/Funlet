-- ============================================================================
-- SETUP ANALYTICS VIEWS AND INDEXES
-- ============================================================================
-- Run this entire file in Supabase SQL Editor to create all analytics views
...
CREATE OR REPLACE VIEW analytics_flow_performance AS
SELECT 
    COALESCE(fc.workflow_name, fd.workflow_name) as workflow_name,
    COALESCE(fc.flows_started, 0) as flows_started,
    COALESCE(fc.flows_completed, 0) as flows_completed,
    COALESCE(fc.completion_rate_percent, 0) as completion_rate_percent,
    COALESCE(fd.drop_off_count, 0) as drop_offs,
    ROUND(100.0 * COALESCE(fd.drop_off_count, 0) / NULLIF(COALESCE(fc.flows_started, 0), 0), 2) as drop_off_rate
FROM (
    SELECT 
        workflow_name,
        COUNT(CASE WHEN event_type = 'flow_started' THEN 1 END) as flows_started,
...
FULL OUTER JOIN (
    ...
    WHERE event_type = 'drop_off'
    GROUP BY workflow_name
) fd ON fc.workflow_name = fd.workflow_name
...
-- SETUP COMPLETE!
-- ============================================================================
-- All views and indexes have been created.
-- You can now run: SELECT * FROM analytics_summary;
-- ============================================================================
