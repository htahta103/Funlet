#!/bin/bash
# AS-044: Stop Auto Sync while paused
# Expected: Organizer stops Auto Sync, same behavior as running
...
# Test: Stop Auto Sync (even if paused, should work the same)
start_time=$(date +%s)
response=$(send_message "2")
rate_limit_sleep

# Validate: Should stop Auto Sync
if validate_test "AS-044" "$response" "stopped\|archived\|cancelled" "" "$start_time"; then
  # Cleanup
