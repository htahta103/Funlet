#!/bin/bash
# AS-045: Invitee replies after stop
# Expected: Invitee replies, "Got it — thanks." sent
...
# Test: Simulate invitee reply after stop
start_time=$(date +%s)
...
if echo "$response_text" | grep -qi "got it\|thanks"; then
  log_test_result "AS-045" "PASS" "Reply after stop handled with thanks message" "$(($(date +%s) - start_time))"
  # Cleanup
  ...
  log_test_result "AS-045" "FAIL" "Reply after stop not handled: $response_text" "$(($(date +%s) - start_time))"
  cleanup_auto_syncs_for_user
  clear_conversation_state
  delete_test_crew "$crew_id"
  exit 1
