#!/bin/bash
# AS-028: Invitee multiple numeric reply
# Expected: Invitee replies 1 and 2 work, both options recorded
...
# Create Auto Sync with multiple options
send_message "auto sync Test Crew" > /dev/null
rate_limit_sleep
send_message "Test Event" > /dev/null
rate_limit_sleep
send_message "1/10 6pm, 1/11 7pm" > /dev/null
rate_limit_sleep
...
# Test: Simulate invitee reply with multiple options
start_time=$(date +%s)
response=$(curl -s -X POST "$FUNCTION_URL" \
  -H "Authorization: Bearer $SUPABASE_ANON_KEY" \
  -H "Content-Type: application/json" \
  -d "{
    \"message\": \"1 and 2 work\",
...
# Validate: Both options should be recorded
response_text=$(extract_response "$response")
if echo "$response_text" | grep -qi "got it\|thanks\|recorded"; then
  log_test_result "AS-028" "PASS" "Multiple options recorded" "$(($(date +%s) - start_time))"
  # Cleanup
  ...
  log_test_result "AS-028" "FAIL" "Multiple options not handled: $response_text" "$(($(date +%s) - start_time))"
  cleanup_auto_syncs_for_user
  clear_conversation_state
  delete_test_crew "$crew_id"
  exit 1
