#!/bin/bash
# AS-029: Invitee replies none
# Expected: Invitee replies none, recorded as not available
...
# Test: Simulate invitee reply "none"
start_time=$(date +%s)
...
# Validate: Should be recorded as not available
response_text=$(extract_response "$response")
if echo "$response_text" | grep -qi "got it\|thanks\|recorded"; then
  log_test_result "AS-029" "PASS" "None reply recorded" "$(($(date +%s) - start_time))"
  # Cleanup
  ...
  log_test_result "AS-029" "FAIL" "None reply not handled: $response_text" "$(($(date +%s) - start_time))"
  cleanup_auto_syncs_for_user
  clear_conversation_state
  delete_test_crew "$crew_id"
  exit 1
