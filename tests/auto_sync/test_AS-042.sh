#!/bin/bash
# AS-042: Invitee replies after invites
# Expected: Invitee replies late, "Got it — thanks." sent
...
# Test: Simulate invitee reply after invites sent
start_time=$(date +%s)
...
# Validate: Should send "Got it — thanks."
response_text=$(extract_response "$response")
if echo "$response_text" | grep -qi "got it\|thanks"; then
  log_test_result "AS-042" "PASS" "Late reply handled with thanks message" "$(($(date +%s) - start_time))"
  # Cleanup
  ...
  log_test_result "AS-042" "FAIL" "Late reply not handled: $response_text" "$(($(date +%s) - start_time))"
  cleanup_auto_syncs_for_user
  clear_conversation_state
  delete_test_crew "$crew_id"
  exit 1
