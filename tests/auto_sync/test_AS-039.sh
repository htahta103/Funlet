#!/bin/bash
# AS-039: Send invites while running
# Expected: Organizer sends invites early, Auto Sync completes immediately
...
# Test: Select Auto Sync (should show management menu)
start_time=$(date +%s)
response=$(send_message "1")
rate_limit_sleep

# Validate: Should show management menu with send invites option
if validate_test "AS-039" "$response" "send invites\|1.*send\|2.*stop" "" "$start_time"; then
  # Cleanup
