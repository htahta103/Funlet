#!/bin/bash
# AS-027: Invitee single numeric reply
# Expected: Invitee replies 1, response recorded correctly
...
# Test: Simulate invitee reply (as invitee, not host)
start_time=$(date +%s)
...
# Validate: Response should be recorded
response_text=$(extract_response "$response")
if echo "$response_text" | grep -qi "got it\|thanks\|recorded"; then
  log_test_result "AS-027" "PASS" "Invitee reply recorded" "$(($(date +%s) - start_time))"
  # Cleanup
  ...
  log_test_result "AS-027" "FAIL" "Invitee reply not handled: $response_text" "$(($(date +%s) - start_time))"
  cleanup_auto_syncs_for_user
  clear_conversation_state
  delete_test_crew "$crew_id"
  exit 1
