-- Create function to trigger auto-launch onboarding on user signup
CREATE OR REPLACE FUNCTION trigger_auto_launch_onboarding()
RETURNS TRIGGER AS $$
DECLARE
  user_phone TEXT;
...
BEGIN
  ...
  IF user_phone IS NOT NULL AND user_phone != '' THEN
    -- Call the auto-launch onboarding function via HTTP
    PERFORM net.http_post(
      url := 'https://jjkduivjlzazcvdeeqde.supabase.co/functions/v1/auto-launch-onboarding',
...
CREATE TRIGGER trigger_auto_launch_onboarding
  ...
  FOR EACH ROW
  EXECUTE FUNCTION trigger_auto_launch_onboarding();

-- Set the service role key (you'll need to set this in your Supabase dashboard)
-- ALTER SYSTEM SET app.settings.service_role_key = 'your-service-role-key';
