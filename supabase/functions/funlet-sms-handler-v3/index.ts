...
import { logWorkflowStart, logCrewCreated, logWorkflowError } from './logger.ts';
  ...
  try {
    ...
    if (extractedParams.crew_name) {
      logWorkflowStart({
        ...
        workflowStep: 'onboarding_start',
        inputData: { crew_name: extractedParams.crew_name }
      });
      ...
      if (crewError) {
        ...
        logWorkflowError({
          ...
          userId,
          workflowName: 'create_crew',
          workflowStep: 'creation_failed',
          errorDetails: { message: crewError.message, code: crewError.code }
        });
        return {
        ...
        if (action === 'CREATE_CREW') {
          ...
          if (!crewName) {
            ...
            logWorkflowStart({
              ...
              userId,
              workflowName: 'create_crew',
              workflowStep: 'initiated_no_name',
              inputData: { source: 'command_no_name' }
            });
          ...
          if (crewName.length < 2) {
            ...
            // Log start if this is a direct command (not continuing from a prompt)
            if (currentState?.waiting_for !== 'crew_name_input') {
              logWorkflowStart({
                supabase,
                userId,
                workflowName: 'create_crew',
                workflowStep: 'initiated_with_name',
                inputData: { crew_name: crewName, source: 'command_with_name' }
              });
            }
